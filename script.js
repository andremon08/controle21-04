...
function preencherListaRelat() {
    // Calcula meses disponíveis direto da fonte de dados
    const mesesSet = new Set();
    ...
    // Preenche o seletor de mês da página de relatórios (só uma vez)
    const relatMesSel = document.getElementById('relatMesSelect');
    if (relatMesSel && relatMesSel.options.length <= 1) {
    ...
    // Define mês atual: o do seletor se já escolhido, senão o mais recente
    if (!_relatMesAtual && mesesOrdenados.length) {
        _relatMesAtual = mesesOrdenados[mesesOrdenados.length - 1];
    }
    if (relatMesSel && _relatMesAtual) relatMesSel.value = _relatMesAtual;
