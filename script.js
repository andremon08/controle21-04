...
function preencherListaRelat() {
    // Preenche o seletor de mês próprio da página de relatórios
    const relatMesSel = document.getElementById('relatMesSelect');
    if (relatMesSel) {
        const mesesOrdenados = (_opsMes && _opsMes.length) ? _opsMes : 
            [...new Set(dadosVerbas.map(l => l['Mês']?.trim()).filter(Boolean))].sort((a, b) => {
                const [ma, aa] = a.split('/').map(Number);
                const [mb, ab] = b.split('/').map(Number);
                return aa !== ab ? aa - ab : ma - mb;
            });

        // Só reconstrói se ainda não foi preenchido
        if (relatMesSel.options.length <= 1) {
            mesesOrdenados.forEach(m => {
                const opt = document.createElement('option');
                opt.value = opt.textContent = m;
                relatMesSel.appendChild(opt);
            });
        }

        // Define o mês atual: usa o do seletor da página de relatórios,
        // senão o mais recente disponível
        if (!_relatMesAtual && mesesOrdenados.length) {
            _relatMesAtual = mesesOrdenados[mesesOrdenados.length - 1];
            relatMesSel.value = _relatMesAtual;
        } else if (_relatMesAtual) {
            relatMesSel.value = _relatMesAtual;
        }
    }
    ...
    // Busca em tempo real — evita adicionar listener duplicado
...
function relatMudouMes() {
    const sel = document.getElementById('relatMesSelect');
    _relatMesAtual = sel.value;
}
